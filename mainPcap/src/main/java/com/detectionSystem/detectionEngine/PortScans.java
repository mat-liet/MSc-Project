 ...
 * @version 2020-08-01
 */
public class PortScans implements Runnable {
     ...
     * @param packet
     */
    public PortScans(Packet packet) {
        this.packet = packet;
    }
    ...
    public void run() {
        ...
        if (isXmasScan(packet)) {
            ...
            boolean seen = false;
            PortscanPacket portscanPacket = new PortscanPacket(id, packetId, msg, packetString, seen);
            DatabaseHandler.getDatabaseInsertionQueue().add(portscanPacket);
    ...
    private boolean isTcp(Packet packet) {
        return (packet.get(IpV4Packet.class).getHeader().getProtocol() == IpNumber.TCP);
    }
