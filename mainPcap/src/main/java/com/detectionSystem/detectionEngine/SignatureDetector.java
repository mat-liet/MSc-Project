...
/**
 * It runs through the ruleslist and checks for signatures in the hex data.
 * @author Matej Lietava
 * @version 2020-08-01
 */
public class SignatureDetector implements Runnable {
     ...
     */
    public SignatureDetector(Packet packet) {
        this.packet = packet;
        this.protocol = getProtocolString();
        this.dstPort = getDstPort();
    }

    @Override
    public void run() {
        // System.out.println("Signature detector on [" + Capture.getIDPacket(packet) +"]");
        String hexStr = getHexString(getBytePayload(packet));
        //Now loop through rules list and check.
        checkForSignatures(hexStr);
     ...
     */
    public void checkForSignatures(String hexStr) {
        for (Map.Entry<Integer, Rule> entry : DetectionHandler.getRuleList().getRules().entrySet()) {
            if (headerMatches(entry.getValue())) {
                if (entry.getValue().getRuleOptions().containsKey("content")) {
                    if (matchesAllSignatures(entry.getValue(), hexStr)) {
                        ...
                        SnortPacket snortPacket = new SnortPacket(0, Capture.getIDPacket(packet), entry.getKey(),getMessage(entry.getValue()),  
                            packet.get(AbstractPacket.class).toString(), false);
                        DatabaseHandler.getDatabaseInsertionQueue().add(snortPacket);                    
                    }
                }
            }
        }
     ...
     * @param rule
     */
    private boolean headerMatches(Rule rule) {
        if (rule.getProtocol().equals(this.protocol) || rule.getProtocol().equals("any")) {
            if (rule.getDstAddress().contains("$HOME_NET") || rule.getDstAddress().contains("any")) {
                if (rule.getDstPort().getPorts().contains(this.dstPort) || rule.getDstPort().getPorts().contains(ANY)) {
                    return true;
                }
            }
        } 
    ...
    private byte[] getBytePayload(Packet packet) {
        return packet.get(IpV4Packet.class).getPayload().getPayload().getRawData();
    }
    ...
    private String getProtocolString() {
        ...
        String protocol = this.packet.get(IpV4Packet.class).getHeader().getProtocol().toString().toLowerCase();
